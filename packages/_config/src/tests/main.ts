...
export type TestSectionFunctsParams = {
	addTest        : ( testOptions: {
		title : string
		fn    : ( args: { expect: ExpectStatic } ) => Promise<void>
	} ) => Promise<void>
	addBooleanTest : ( testOptions: {
		title    : string
		expected : boolean
		fn       : ( ) => Promise<boolean>
	} ) => Promise<void>
...
export const test = ( fn: ( args: TestParams ) => Promise<void> ) => {
	...
	const section = async ( {
		...
		describe( title, () => {
			...
			const addTest: TestSectionFunctsParams['addTest'] = async ( {
				title, fn,
			} ) => {
			...
			const addBooleanTest: TestSectionFunctsParams['addBooleanTest'] = async ( {
				title, fn, expected,
			} ) => {
